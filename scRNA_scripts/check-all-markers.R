...
if(sp=='human'){
   lapply(markers, function(x){
     ...
     ggsave(paste('check_for_',x,'.pdf'),height = h, width = z_width / 2)
   })
  lapply(markers_list, function(x){
    ...
    ggsave(paste('check_for_',x,'.pdf'), height =  z_width / 2 ,width  = w)
  })
...
ggsave(p,filename = paste('last_markers_and_umap.pdf'),width  = w + 4,height = h)
...
p3=FeatureScatter(sce.all.int , "nCount_RNA", "nFeature_RNA", 
                  pt.size = 0.5)
ggsave(filename=paste0(pro,"Scatterplot.pdf"),plot=p3, height = 6, width = 6)
...
if(T){
  ...
  p1 <- DoHeatmap(  sce.Scale , top_10 , 
              size=3)
  
  ggsave(p1, filename=paste0(pro,'DoHeatmap_check_top10_markers_by_clusters.pdf') ,
         # limitsize = FALSE,
         units = "cm",width=width,height=height)
  width <- 8+0.6*length(unique(Idents(sce.all.int)));width
  height <- 8+0.2*length(top_10);height
  p1 <- DotPlot(sce.all.int, features = top_10 ,
          assay='RNA'  )  + coord_flip() +FontSize(y.text = 4)
  ggsave(p1, filename =  paste0(pro,'DotPlot_check_top10_markers_by_clusters.pdf'),
         units = "cm",width=width,height=height)
  ...
  p1 <- DoHeatmap(  sce.Scale , top_3 , 
              size=3)
  ggsave(p1, filename=paste0(pro,'DoHeatmap_check_top3_markers_by_clusters.pdf') ,
         units = "cm",width=width,height=height)
  
  width <- 8+0.2*length(unique(Idents(sce.all.int)));width
  height <- 8+0.1*length(top_3);height
  p1 <- DotPlot(sce.all.int, features = top_3 ,
          assay='RNA'  )  + coord_flip()
  ggsave(p1, filename = paste0(pro,'DotPlot_check_top3_markers_by_clusters.pdf'),width=width,height=height)
}

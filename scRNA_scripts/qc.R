basic_qc <- function(input_sce){
  ...
  #计算红血细胞基因比例
  Hb_genes=rownames(input_sce)[grep("^Hb[abq]", rownames(input_sce),ignore.case = T)]
  print(Hb_genes)
  ...
  p1=VlnPlot(input_sce, group.by = "sample", layer = "counts",  # 显式指定层
             features = feats, pt.size = 0, ncol = 2) + 
    NoLegend() &
    theme(axis.text.x = element_text(size = 8))  # 调整X轴文本大小
  p1 
  w=length(unique(input_sce$orig.ident))/3+5;w
  ggsave(filename="Vlnplot1.pdf",plot=p1,width = w,height = 5)
  
  feats <- c("percent_mito", "percent_ribo", "percent_hb")
  p2=VlnPlot(input_sce, group.by = "sample", features = feats, pt.size = 0, ncol = 3, same.y.lims=T) + 
  ...
  p3=FeatureScatter(input_sce, "nCount_RNA", "nFeature_RNA", group.by = "sample", pt.size = 0.5)
  ...
  # 先走默认流程即可
  if(T){
    selected_c <- WhichCells(input_sce, expression = nFeature_RNA > 400 & nFeature_RNA  < 7000 & nCount_RNA > 500 & nCount_RNA < 50000)
    #selected_c <- WhichCells(input_sce, expression = nFeature_RNA > 400 & nFeature_RNA  < 8000 & nCount_RNA > 500 & nCount_RNA < 50000)
  ...
  #from 400 to 8000, and number of UMIs from 500 to 50000
  ...
  selected_mito <- WhichCells(input_sce.filt, expression = percent_mito < 10)
  ...
  table(input_sce.filt$sample) 
  
  #可视化过滤后的情况
  feats <- c("nFeature_RNA", "nCount_RNA", "percent_mito")
  p1_filtered=VlnPlot(input_sce.filt, group.by = "sample", features = feats, pt.size = 0, ncol = 2) + 
    NoLegend()
  w=length(unique(input_sce.filt$orig.ident))/3+8;w 
  ggsave(filename="Vlnplot1_filtered.pdf",plot=p1_filtered,width = w,height = 10)
